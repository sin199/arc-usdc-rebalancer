...
launchd_label="${LIVE_FOLLOW_CONSUME_LAUNCHD_LABEL:-$label_base.consume}"
...
if [[ -f "$launchd_plist" ]]; then
  if launchctl print "gui/$UID/$launchd_label" >"$launchd_tmp" 2>&1; then
    ...
    rm -f "$launchd_tmp"
    echo "live_follow_consume status=running pid=${pid:-none} mode=launchd state=${state:-unknown} label=$launchd_label log=$RUN_LOG"
    exit 0
  fi
...
if command -v screen >/dev/null 2>&1; then
  if screen -ls 2>/dev/null | grep -Eq "[[:space:]]+[0-9]+\\.${session}[[:space:]]"; then
    ...
    echo "live_follow_consume status=running pid=${screen_pid:-unknown} mode=${mode:-screen} screen_session=$session log=$RUN_LOG"
    exit 0
  fi
fi

if [[ ! -f "$PID_FILE" ]]; then
  echo "live_follow_consume status=stopped"
  exit 0
fi

pid="$(cat "$PID_FILE" 2>/dev/null || true)"
if [[ -z "${pid:-}" ]]; then
  echo "live_follow_consume status=stopped reason=empty_pid_file"
  exit 0
fi

if kill -0 "$pid" 2>/dev/null; then
  mode="$(cat "$MODE_FILE" 2>/dev/null || echo unknown)"
  echo "live_follow_consume status=running pid=$pid mode=$mode log=$RUN_LOG"
  exit 0
fi

rm -f "$PID_FILE" "$MODE_FILE"
echo "live_follow_consume status=stopped reason=stale_pid pid=$pid"
exit 0
