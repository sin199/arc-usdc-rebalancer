...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "${old_pid:-}" ]] && kill -0 "$old_pid" 2>/dev/null; then
    ...
    echo "live_follow_ingest already running pid=$old_pid mode=$mode log=$RUN_LOG"
    exit 0
  fi
...
if ! [[ "$interval" =~ ^[0-9]+$ ]] || (( interval < 10 )); then
  echo "invalid LIVE_FOLLOW_INGEST_INTERVAL_SECONDS=$interval" >&2
  exit 1
fi
...
if command -v screen >/dev/null 2>&1; then
  ...
  if screen -ls 2>/dev/null | grep -Eq "[[:space:]]+[0-9]+\\.${session}[[:space:]]"; then
    ...
    echo "live_follow_ingest already running pid=${screen_pid:-unknown} mode=screen_${interval}s log=$RUN_LOG"
    exit 0
  fi
  ...
  if [[ -n "${screen_pid:-}" ]]; then
    ...
    echo "screen_${interval}s" > "$MODE_FILE"
    echo "live_follow_ingest started pid=$screen_pid mode=screen_${interval}s log=$RUN_LOG"
    exit 0
  fi

  rm -f "$PID_FILE" "$MODE_FILE"
  echo "live_follow_ingest failed to start screen session=$session; check $RUN_LOG" >&2
  exit 1
fi
...
if kill -0 "$new_pid" 2>/dev/null; then
  echo "live_follow_ingest started pid=$new_pid mode=interval_${interval}s log=$RUN_LOG"
  exit 0
fi

rm -f "$PID_FILE" "$MODE_FILE"
echo "live_follow_ingest failed to start; check $RUN_LOG" >&2
exit 1
