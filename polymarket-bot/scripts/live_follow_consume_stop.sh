...
if [[ ! -f "$PID_FILE" ]]; then
  if (( stopped_launchd )); then
    ...
    echo "live_follow_consume stopped launchd_label=$launchd_label"
  else
    echo "live_follow_consume stop checked screen_session=$session pid_file=missing"
  fi
  exit 0
fi
...
if [[ -z "${pid:-}" ]]; then
  rm -f "$PID_FILE" "$MODE_FILE"
  echo "live_follow_consume not running (empty pid file)"
  exit 0
fi

if ! kill -0 "$pid" 2>/dev/null; then
  rm -f "$PID_FILE" "$MODE_FILE"
  echo "live_follow_consume not running (stale pid=$pid)"
  exit 0
fi
...
for _ in $(seq 1 10); do
  if ! kill -0 "$pid" 2>/dev/null; then
    rm -f "$PID_FILE" "$MODE_FILE"
    echo "live_follow_consume stopped pid=$pid"
    exit 0
  fi
  sleep 1
done
...
rm -f "$PID_FILE" "$MODE_FILE"
echo "live_follow_consume force-stopped pid=$pid"
