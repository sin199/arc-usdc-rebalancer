...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "${old_pid:-}" ]] && kill -0 "$old_pid" 2>/dev/null; then
    ...
    echo "live_follow_hourly_report already running pid=$old_pid mode=$mode log=$RUN_LOG"
    exit 0
  fi
...
if command -v screen >/dev/null 2>&1; then
  ...
  if screen -ls 2>/dev/null | grep -Eq "[[:space:]]+[0-9]+\\.${session}[[:space:]]"; then
    ...
    echo "live_follow_hourly_report already running pid=${screen_pid:-unknown} mode=screen_hourly log=$RUN_LOG"
    exit 0
  fi
  ...
  if [[ -n "${screen_pid:-}" ]]; then
    ...
    echo "screen_hourly" > "$MODE_FILE"
    echo "live_follow_hourly_report started pid=$screen_pid mode=screen_hourly log=$RUN_LOG"
    exit 0
  fi

  rm -f "$PID_FILE" "$MODE_FILE"
  echo "live_follow_hourly_report failed to start screen session=$session; check $RUN_LOG" >&2
  exit 1
fi
...
echo "interval_hourly" > "$MODE_FILE"

sleep 1
if kill -0 "$new_pid" 2>/dev/null; then
  echo "live_follow_hourly_report started pid=$new_pid mode=interval_hourly log=$RUN_LOG"
  exit 0
fi

rm -f "$PID_FILE" "$MODE_FILE"
echo "live_follow_hourly_report failed to start; check $RUN_LOG" >&2
exit 1
