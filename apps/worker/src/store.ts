...
export type RobotStore = {
  ...
  update(
    ...
    ) => RobotRuntimeState | Promise<RobotRuntimeState>,
...
function legacyStatusToJobStatus(
...
function timelineForLegacyJob(
  ...
  if (status !== 'created') {
    timeline.push({
      ...
      at: updatedAt,
      actor:
        status === 'rejected' || status === 'cancelled' ? 'operator' : 'robot',
...
function migrateLegacyState(raw: Record<string, unknown>): RobotRuntimeState {
  ...
  const jobs = raw.latestRuns
    .map((run) => {
      ...
      const status = legacyStatusToJobStatus(legacyStatus)
      const simulatedLegacy =
        legacyStatus === 'submitted' || legacyStatus === 'confirmed'
      const executorRecord =
        record.executor &&
        typeof record.executor === 'object' &&
        record.executor !== null
          ? (record.executor as Record<string, unknown>)
          : undefined
      const executorName = simulatedLegacy
        ? 'none'
        : ((executorRecord?.name as
            | 'local'
            | 'circle'
            | 'bridge'
            | 'none'
            | undefined) ?? 'none')
      const type = legacyKindToJobType(
        typeof record.kind === 'string' ? record.kind : undefined,
      )
      const amountUsdc =
        typeof record.amountUsdc === 'number'
          ? record.amountUsdc
          : Number(record.amountUsdc ?? 0)
      const balanceUsdc =
        typeof record.balanceUsdc === 'number'
          ? record.balanceUsdc
          : Number(record.balanceUsdc ?? 0)
      const policyMinThresholdUsdc =
        typeof record.minThresholdUsdc === 'number'
          ? record.minThresholdUsdc
          : Number(record.minThresholdUsdc ?? 0)
      const policyTargetBalanceUsdc =
        typeof record.targetBalanceUsdc === 'number'
          ? record.targetBalanceUsdc
          : Number(record.targetBalanceUsdc ?? 0)
      ...
      return {
        ...
        requestedAction: {
          summary:
            typeof record.reason === 'string'
              ? record.reason
              : 'Legacy job imported from previous execution state.',
          triggerSource:
            record.triggerSource === 'manual' ||
            record.triggerSource === 'approval' ||
            record.triggerSource === 'startup'
        ...
        parameters: {
          ...
          minThresholdUsdc: Number.isFinite(policyMinThresholdUsdc)
            ? policyMinThresholdUsdc
            : 0,
          targetBalanceUsdc: Number.isFinite(policyTargetBalanceUsdc)
            ? policyTargetBalanceUsdc
            : 0,
          destinationAddress:
            typeof record.destinationAddress === 'string' &&
            record.destinationAddress.startsWith('0x')
            ...
            ? ((record.recipients as Array<Record<string, unknown>>)
                .map((recipient) => {
                  ...
                  const recipientAmount =
                    typeof recipient.amountUsdc === 'number'
                      ? recipient.amountUsdc
                      : Number(recipient.amountUsdc ?? 0)

                  return {
                    address,
                    amountUsdc: Number.isFinite(recipientAmount)
                      ? recipientAmount
                      : 0,
        ...
        riskChecks: Array.isArray(record.reasonCodes)
          ...
          : [],
        executionMode:
          record.mode === 'manual-approve' || record.mode === 'auto'
            ? record.mode
            : ('dry-run' as const),
        status,
        approvalRequired:
        ...
        updatedAt,
        policyAddress:
          typeof record.policyAddress === 'string' &&
          record.policyAddress.startsWith('0x')
            ? (record.policyAddress as `0x${string}`)
            : ('0x0000000000000000000000000000000000000000' as `0x${string}`),
        treasuryAddress:
          typeof record.treasuryAddress === 'string' &&
          record.treasuryAddress.startsWith('0x')
            ? (record.treasuryAddress as `0x${string}`)
            : ('0x0000000000000000000000000000000000000000' as `0x${string}`),
        balanceUsdc: Number.isFinite(balanceUsdc) ? balanceUsdc : 0,
        policyMinThresholdUsdc: Number.isFinite(policyMinThresholdUsdc)
          ? policyMinThresholdUsdc
          : 0,
        policyTargetBalanceUsdc: Number.isFinite(policyTargetBalanceUsdc)
          ? policyTargetBalanceUsdc
          : 0,
        policyMaxRebalanceAmountUsdc: Number.isFinite(
          policyMaxRebalanceAmountUsdc,
        )
          ? policyMaxRebalanceAmountUsdc
          : 0,
        txHash:
          simulatedLegacy || typeof record.txHash !== 'string'
            ? undefined
            : record.txHash,
        txUrl:
          simulatedLegacy || typeof record.txUrl !== 'string'
            ? undefined
            : record.txUrl,
        result: simulatedLegacy
          ? 'Migrated legacy local execution record. No chain transaction was submitted.'
        ...
        executor: {
          ...
          txHash:
            ...
            record.executor !== null
              ? simulatedLegacy
                ? undefined
                : typeof (record.executor as Record<string, unknown>).txHash ===
                    'string'
                  ? ((record.executor as Record<string, unknown>)
                      .txHash as string)
                  : undefined
          ...
          txUrl:
            ...
            record.executor !== null
              ? simulatedLegacy
                ? undefined
                : typeof (record.executor as Record<string, unknown>).txUrl ===
                    'string'
                  ? ((record.executor as Record<string, unknown>)
                      .txUrl as string)
                  : undefined
          ...
          error:
            ...
            typeof record.executor === 'object' &&
            record.executor !== null
              ? typeof (record.executor as Record<string, unknown>).error ===
                'string'
        ...
        logs: Array.isArray(record.logs)
          ? record.logs.map((line) => String(line))
          : [],
  ...
  const state: RobotRuntimeState = {
    ...
    robot: buildRobotIdentity({
      mode:
        raw.mode === 'manual-approve' || raw.mode === 'auto'
          ? raw.mode
          : 'dry-run',
...
function normalizeState(raw: unknown): RobotRuntimeState {
  ...
  if (record.version === 2 && Array.isArray(record.jobs)) {
    const jobs = (record.jobs as TreasuryJobRecord[]).map((job) => {
      if (
        job.status !== 'simulated' &&
        !(
          job.executor?.name === 'local' &&
          (job.status === 'submitted' || job.status === 'confirmed')
        )
      ) {
        return job
      }
      ...
      return {
        ...
        txUrl: undefined,
        result:
          'Migrated local execution record. No chain transaction was submitted.',
        executor: {
          ...job.executor,
          name: 'none' as const,
          enabled: false,
    ...
    return {
      ...
      robot: {
        ...createDefaultIdentity(
          (record.mode as RobotRuntimeState['mode']) ?? 'dry-run',
        ),
  ...
  async function withFileLock<T>(task: () => Promise<T>): Promise<T> {
    ...
    while (true) {
      try {
        ...
        await writeFile(`${lockPath}/owner`, `${lockOwner}\n`)
        break
      } catch (error) {
        ...
        let stale = false
        try {
          const owner = (await readFile(`${lockPath}/owner`, 'utf8')).trim()
          ...
          stale = Date.now() > deadline - 29_000
        }
        ...
        if (Date.now() >= deadline) {
          throw new Error(
            `Timed out waiting for execution state lock: ${lockPath}`,
          )
        }
