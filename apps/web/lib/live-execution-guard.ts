...
import {
  ...
  type LiveExecutionIntent,
} from './live-execution-auth'
import {
  getLiveExecutionGuardMode,
  reserveLiveExecutionRequest,
...
type AuthorizationResult =
  | { ok: true; authorization: LiveExecutionAuthorization }
...
export function getLiveExecutionStatus() {
  const allowlist = readCsv('LIVE_EXECUTION_OPERATOR_ALLOWLIST').filter(
    (item): item is Address => isAddress(item),
  )
...
export function auditLiveExecution(
  event: string,
...
function isAllowedOrigin(request: NextRequest, origin: string) {
  ...
  const allowedOrigins =
    configuredOrigins.length > 0 ? configuredOrigins : [request.nextUrl.origin]
...
export async function authorizeLiveExecution(
  ...
  if (!status.operatorAllowlistConfigured) {
    return {
      ...
      response: reject(
        403,
        'Live execution operator allowlist is not configured.',
        intent.requestId,
      ),
    }
  }

  if (status.guardMode === 'unavailable') {
    return {
      ok: false,
      response: reject(
        503,
        'Durable live execution replay protection is not configured.',
        intent.requestId,
      ),
    }
  }
  ...
  if (!isAddress(intent.operatorAddress)) {
    return {
      ...
      response: reject(
        400,
        'A valid operator address is required.',
        intent.requestId,
      ),
    }
  }
  ...
  const allowlist = new Set(
    readCsv('LIVE_EXECUTION_OPERATOR_ALLOWLIST').map((item) =>
      item.toLowerCase(),
    ),
  )
  if (!allowlist.has(intent.operatorAddress.toLowerCase())) {
    return {
      ...
      response: reject(
        403,
        'The connected operator is not authorized.',
        intent.requestId,
      ),
    }
  }
  ...
  const freshnessWindowMs =
    readPositiveNumber('LIVE_EXECUTION_SIGNATURE_TTL_SECONDS', 60) * 1_000
  const now = Date.now()

  if (
    !Number.isFinite(intent.timestamp) ||
    intent.timestamp > now + 5_000 ||
    now - intent.timestamp > freshnessWindowMs
  ) {
    return {
      ok: false,
      response: reject(
  ...
  if (recoveredAddress.toLowerCase() !== intent.operatorAddress.toLowerCase()) {
    return {
      ...
      response: reject(
        401,
        'The wallet signature does not match the operator.',
        intent.requestId,
      ),
    }
  }
  ...
  if (intent.kind === 'execute') {
    if (!Number.isFinite(intent.amountUsdc) || (intent.amountUsdc ?? 0) <= 0) {
      return {
        ...
        response: reject(
          400,
          'Execution amount must be positive.',
          intent.requestId,
        ),
      }
    }
    ...
    if ((intent.amountUsdc ?? 0) > status.maxAmountUsdc) {
      return {
        ...
        response: reject(
          400,
          `Execution amount exceeds the ${status.maxAmountUsdc} USDC deployment limit.`,
          intent.requestId,
        ),
      }
    }
  ...
  const rateLimit = Math.max(
    1,
    Math.floor(readPositiveNumber('LIVE_EXECUTION_RATE_LIMIT_PER_MINUTE', 3)),
  )
  const operatorKey = recoveredAddress.toLowerCase()
  const reservation = await reserveLiveExecutionRequest({
    now,
    operatorKey,
    rateLimit,
  ...
  if (!reservation.ok && reservation.reason === 'rate_limited') {
    return {
      ok: false,
      response: reject(
        429,
        'The live execution rate limit has been reached.',
        intent.requestId,
      ),
    }
  }
  ...
  audit('live_execution_authorized', {
    guardMode: reservation.ok ? reservation.mode : status.guardMode,
    kind: intent.kind,
